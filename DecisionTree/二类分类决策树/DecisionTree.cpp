...
int DecisionTree::statister(vector<vector<string>>& infos, vector<attributes*>& statTree, 
							set<int>& readLine, vector<int>& readClumNum)//readLine就是记录每一行行号的容器
{
	//yes的总行数
	int deciNum = 0;
	//统计每一行
		...
		for (int i = 0; i < attriNum - 1; i++)
		{
			if (readClumNum[i] == 0)
			{
				...
				if (map_iter == (statTree[i]->attriItem).end())
				{
					...
					decisLine ? attritem->itemNum.push_back(1) : attritem->itemNum.push_back(0);//如果分类是yes则输入1,否则加入0
				...
				else
				{
					...
					if(decisLine)
					{
						(map_iter->second)->itemNum[1]++;//更新包含的yes数目
					}
				}
			}
		}
	...
	//	}
			...
			map_iterator != statTree[attr_node]->attriItem.end(); ++map_iterator)
		{
			...
			else
			{
				...
				{
					cout <<statTree[attr_node]->attriName<< "的分支--"<< map_iterator->first <<"的叶子---"<<treeLine << ">no" << " " << sum << endl;
				}
				else
				{
					cout <<statTree[attr_node]->attriName<< "的分支--"<< map_iterator->first <<"的叶子---"<< treeLine  << ">yes" << " " << deci_Num <<endl;
				}
			}
		}
		//还原属性列可用性
